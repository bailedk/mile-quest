...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_team_goals_waypoints ON team_goals USING GIN (waypoints);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_team_goals_route_data ON team_goals USING GIN ("routeData");

-- Add expression index for route bounds queries (common for map viewport calculations)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_team_goals_route_bounds ON team_goals USING GIN (("routeData"->'bounds'));

-- Add partial index for active goals with distance (improves active goal queries)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_team_goals_active ON team_goals (teamId, targetDistance) 
WHERE status = 'ACTIVE';

-- Add index for completed goals (useful for historical queries)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_team_goals_completed ON team_goals (teamId, completedAt DESC) 
WHERE status = 'COMPLETED';

-- Add covering index for goal progress queries
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_team_goals_progress ON team_goals (id, teamId, targetDistance, status) 
INCLUDE (name, routePolyline, startLocation, endLocation);

-- Add index for date range queries with status
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_team_goals_date_status ON team_goals (startDate, endDate, status) 
